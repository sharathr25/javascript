...
The pattern is said to be present in the grid. The return value should be YES or NO, depending on whether the pattern is found. In this case, return YES.
...
function gridSearch (G, P) {
  for (let i = 0; i < G.length; i++) {
    ...
    let lastndex
    for (; j < P.length; j++) {
      if (i + j === G.length) break
      const index = G[i + j].indexOf(P[j])
      if (index === -1) break
      lastndex = index
    }
    j = 0
    for (; j < P.length; j++) {
      if (i + j === G.length) break
      const index = G[i + j].indexOf(P[j], maxIndex)
      if (index === -1) break
    }
